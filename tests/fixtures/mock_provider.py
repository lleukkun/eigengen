...
from eigengen.providers import Provider, Model, ModelPair
...
class MockProvider(Provider):
    ...
    def make_request(self,
                     ...
                     temperature: float,
                     max_retries: int = 5,
                     base_delay: int = 1) -> Generator[str, None, None]:
...
def create_mock_model_pair(canned_responses: Dict[str, str]) -> ModelPair:
    model_pair = ModelPair(large=Model(provider=MockProvider(canned_responses), model_name="mock_large", temperature=0.7, max_tokens=8192),
